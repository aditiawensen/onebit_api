...
if ($lastId < 0) {
	$q = $con->query("SELECT * FROM obrolan, pengguna WHERE obrolan.id_perangkat=pengguna.id_pengguna ORDER BY waktu_obrolan DESC LIMIT ".$startRow.",".$limitRow);
} else {
	$q = $con->query("SELECT * FROM obrolan, pengguna WHERE obrolan.id_perangkat=pengguna.id_pengguna AND id_obrolan <= ".$lastId." ORDER BY waktu_obrolan DESC LIMIT ".$startRow.",".$limitRow);
}
